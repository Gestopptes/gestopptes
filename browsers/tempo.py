...
@activity.defn
async def compose_greeting(input: Url) -> str:
    activity.logger.info("PENIS 1 GREETING %s" % input)
    return f"{input.url}!"
...
class GreetingWorkflow:
    ...
    async def run(self, name: str) -> str:
        workflow.logger.info("PENIS 2 WORKFLOW %s" % name)
...
DEMO_QUEUE = "hello-activity-task-queue"
